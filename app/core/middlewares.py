...
class HttpAuditLogMiddleware(BaseHTTPMiddleware):
    ...
    async def get_request_args(self, request: Request) -> dict:
        ...
        if request.url.path == "/api/v1/ionosphere/datasets/upload":
            # The endpoint counts/limits raw bytes while streaming to disk. Audit must not
            # consume this body (request.json/form would buffer a potentially 1GiB file).
            args["contentType"] = request.headers.get("content-type", "")
            args["contentLength"] = request.headers.get("content-length")
            return args
    ...
    async def get_response_body(self, request: Request, response: Response) -> Any:
        ...
        if content_type == "application/octet-stream":
            # Keep binary responses intact and avoid persisting raw bytes in a JSONField.
            return {"contentType": content_type, "byteLength": int(content_length) if content_length else None}
