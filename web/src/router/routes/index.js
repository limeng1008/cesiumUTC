...
export const basicRoutes = [
  ...
  ...systemModules.map((module, order) => ({
    name: `Module-${module.key}`,
    path: module.path,
    component: Layout,
    meta: { order, moduleKey: module.key },
    children: [
      {
        path: '',
        name: `Module-${module.key}-view`,
        component:
          module.key === 'datasets'
            ? () => import('@/views/data-management/index.vue')
            : module.key === 'monitor'
            ? () => import('@/views/ionosphere/volume/index.vue')
            : module.key === 'analysis'
            ? () => import('@/views/analysis/index.vue')
            : module.key === 'temporal'
            ? () => import('@/views/time-variation/index.vue')
            : module.available
            ? () => import('@/views/cesium/index.vue')
            : () => import('@/views/pending/index.vue'),
        meta: { title: module.title, moduleKey: module.key },
      },
    ],
  })),
  { path: '/cesium-test', redirect: '/globe', isHidden: true },
  {
